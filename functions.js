...
function addPlayer(startingBet) {
	...
	const index = 10 - names.length

	playerArea.innerHTML += '<tr><td><label for=' + id + '>' + name + '</label></td><td><input id=' + id + ' type=number value=' + startingBet + ' disabled></td><td><input class=current type=number min=0 value=0 tabindex=' + index + ' onfocus="this.oldvalue = this.value" onchange="adjustValues(this, ' + id + ')"></td><td><button onclick="assignBet(' + id + ')">Won</button></td></tr>'
}
